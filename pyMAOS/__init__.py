...
def debug(msg, *args, **kwargs):
	log.debug(f"\033[34m{msg}\033[0m", *args, **kwargs)
...
# %% Print warning messages in orange for visibility
def warning(msg, *args, **kwargs):
	log.warning(f"\033[33m{msg}\033[0m", *args, **kwargs)


# %% Error logging with red color for visibility
def error(msg, *args, **kwargs):
	log.error(f"\033[31m{msg}\033[0m", *args, **kwargs)


#%% Critical logging with bold red color for visibility
def critical(msg, *args, **kwargs):
    log.critical(f"\033[1;31m{msg}\033[0m", *args, **kwargs)
...
np.set_printoptions(precision=2, threshold=999, linewidth=120, suppress=True,
                    formatter={
	                    np.float32: lambda x: '.'.center(10) if abs(x) < 1e-10 else f"{x:10.4g}"})  # type: ignore
...
__all__ = [
	...
	'INTERNAL_FORCE_UNIT', 'INTERNAL_LENGTH_UNIT', 'INTERNAL_MOMENT_UNIT', 'INTERNAL_PRESSURE_UNIT',
	'INTERNAL_DISTRIBUTED_LOAD_UNIT',
	'FORCE_DIMENSIONALITY', 'LENGTH_DIMENSIONALITY', 'MOMENT_DIMENSIONALITY', 'PRESSURE_DIMENSIONALITY',
	'DISTRIBUTED_LOAD_DIMENSIONALITY',
