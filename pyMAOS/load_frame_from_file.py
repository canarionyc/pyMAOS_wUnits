...
def load_frame_from_file(filename, logger=None, schema_file=None, show_vtk=False):
	...
	for member_data in data.get("members", []):
		...
		assert inode is not None, f"{inode} does not exist"
		inode.is_inode_of_elem_ids.append(member_id)

		j_node = member_data["j_node"];
		assert j_node is not None, f"{j_node} does not exist"
		...
		inode.is_inode_of_elems.append(element);
		jnode.is_inode_of_elems.append(element)
	...
	for member_load in data.get("member_loads", []):
		...
		if load_type == 3:  # Distributed load
			...
			if "b_pct" in member_load:
				...
				b_with_units = pyMAOS.unit_manager.ureg.Quantity(member_load.get("b", str(element.length))).to(
					INTERNAL_LENGTH_UNIT)

			element.add_distributed_load(w1_with_units, w2_with_units, a_with_units, b_with_units, load_case,
			                             direction=direction)
			...
			if a_with_units > element.length:
				pyMAOS.info(
					f"Warning: Point load position {a_with_units} exceeds element length {element.length}. Clamping to length.")
			...
			m = m_with_units.to(internal_moment_unit).magnitude if isinstance(m_with_units,
			                                                                  pint.Quantity) else m_with_units
...
def load_linear_load_reactively(element, member_load, logger=None):
	...
	from pyMAOS import INTERNAL_DISTRIBUTED_LOAD_UNIT
