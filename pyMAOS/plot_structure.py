...
def plot_structure(
    ...
    # --- Add legend for section colors to the geometry plot ---
    # This assumes your Section object has a 'name' attribute. If not, you might use `section.uid` or another identifier.
    legend_elements = [Line2D([0], [0], color=color, lw=2, label=f'Section: {section.name if hasattr(section, "name") else section.uid}')
                       for section, color in section_color_map.items()]
