...
def load_frame_from_json(filename, logger=None):
    ...
    log(f"Read {len(nodes_dict)} nodes."); log(str(nodes_dict))
    ...
    def process_materials(materials_data) -> dict:
        ...
        for material_data in materials_data:
            ...
            # Always convert to SI units (Pascal)
            if isinstance(e_value_with_units, pint.Quantity):
                try:
                    # Try converting to Pascal (SI unit)
                    e_value = e_value_with_units.to('Pa').magnitude
                    log(f"Converted material E from {e_value_with_units} to {e_value} Pa")
                except Exception as e1:
                    try:
                        # Try the expanded form as a fallback
                        e_value = e_value_with_units.to('N/m^2').magnitude
                        log(f"Converted material E to {e_value} Pa using expanded form N/m²")
                    except Exception as e2:
                        # If all conversions fail, use raw value
                        log(f"Warning: Could not convert {e_value_with_units} to Pa, using raw value")
                        e_value = e_value_with_units.magnitude
            else:
                e_value = e_value_with_units
                log(f"No units specified for material E, assuming {e_value} Pa")
            
            material = Material(uid=material_id, E=float(e_value))  # Explicitly cast to float
            materials_dict[material_id] = material
            log(f"Processed material {material_id}: E={material.E} Pa")
        return materials_dict

    materials_dict = process_materials(data.get("materials", []))

    # Process sections - always convert to SI units
    def process_sections(sections_data) -> dict:
        sections_dict = {}
        for section_data in sections_data:
            ...
            # Convert to SI units (m^2 for area, m^4 for moment of inertia)
            if isinstance(area_with_units, pint.Quantity):
                try:
                    area = float(area_with_units.to('m^2').magnitude)
                    log(f"Converted section area from {area_with_units} to {area} m²")
                except:
                    area = float(area_with_units.magnitude)
                    log(f"Warning: Could not convert area {area_with_units} to m², using raw value")
            else:
                area = float(area_with_units)
                log(f"No units specified for section area, assuming {area} m²")
        
            if isinstance(ixx_with_units, pint.Quantity):
                try:
                    ixx = float(ixx_with_units.to('m^4').magnitude)
                    log(f"Converted section inertia from {ixx_with_units} to {ixx} m⁴")
                except:
                    ixx = float(ixx_with_units.magnitude)
                    log(f"Warning: Could not convert {ixx_with_units} to m⁴, using raw value")
            else:
                ixx = float(ixx_with_units)
                log(f"No units specified for section moment of inertia, assuming {ixx} m⁴")
        
            section = Section(uid=section_id, Area=area, Ixx=ixx)
            sections_dict[section_id] = section
        return sections_dict
    
    sections_dict = process_sections(data.get("sections", []))
    ...
    for member_load in data.get("member_loads", []):
        ...
        try:
            if load_type == 3:  # Distributed load
                ...
                w1 = w1_with_units.to('N/m').magnitude if isinstance(w1_with_units, pint.Quantity) else w1_with_units
                w2 = w2_with_units.to('N/m').magnitude if isinstance(w2_with_units, pint.Quantity) else w2_with_units
...
if __name__ == "__main__":
    ...
    from pyMAOS.units_mod import (
        SI_UNITS, IMPERIAL_UNITS, METRIC_KN_UNITS, set_unit_system,
        DISPLAY_UNITS
    )

    # Choose unit system with a simple function call
    logger.info(f"\nSetting {args.units} unit system...")
    if args.units == "imperial":
        set_unit_system(IMPERIAL_UNITS, args.units)
    ...
    model_structure = R2Struct.R2Structure(node_list, element_list)
